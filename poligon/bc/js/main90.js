...
function sendRequest(req, data, errorEvent) {
  try {
    req.open('POST', 'https://www.bestchange.net/action.php', true);
...
function goto_list(from, to) {
  ...
  openDocument('https://www.bestchange.net/' + cu_list[id2pos(fromcur)] + '-to-' + cu_list[id2pos(tocur)] + '.html', false);
}
...
function make_tablink(direct) {
  ...
  for (var i = 0; i < ds_list.length; i++) {
    ...
    if (a_id.name) a_id.href = 'https://www.bestchange.net/' + a_id.name;
  }
}
...
function error_upd_img() {
  change_upd_img('', 'https://www.bestchange.net/images/error.png');
  setTimeout(function () {
    change_upd_img('hide');
  }, 500);
}
...
function ccl(id, from, to, url) {
  ...
  if (!url) url = 1;
  last_exch_id = id;
  openDocument('https://www.bestchange.net/click.php?id=' + id + (from && to ? '&from=' + from + '&to=' + to : '') + '&url=' + url, true);
...
function open_reviews(id) {
  if (typeof direct_data != 'undefined' && direct_data) {
    set_cur_cookies();
    openDocument('https://www.bestchange.net/' + direct_data[id]['u'] + '-exchanger.html', false);
  }
  return false;
}
...
function set_status(name, status, filename, noclear) {
  ...
  status_timer = 0;
  var img_content = status != '' ? '<img src="https://www.bestchange.net/images/' + filename + '" width="16" height="16" />' : '';
...
function show_info(id) {
  if (typeof direct_data != 'undefined' && direct_data) {
    if (id !== show_box) {
      ...
      nodeById('det_changer').innerHTML = '<a target="_blank" href="https://www.bestchange.net/click.php?' + direct_data[id]['a'] + '" title="Открыть обменник">' + direct_data[id]['n'] + '</a>' + (direct_data[id]['h'] ? '<span class="https" title="Соединение с сайтом защищено">HTTPS</span>' : '') + (star_content ? '<span class="stars" title="Рейтинг обменника">' + star_content + '</span>' : '');
      ...
      nodeById('det_country').innerHTML = '<img src="https://www.bestchange.net/images/countries/' + direct_data[id]['c'] + '.png" valign="middle" />' + direct_data[id]['m'];
    }
  }
}
...
function update_rates() {
  ...
  if (update_request) {
    change_upd_img('', 'https://www.bestchange.net/images/ajax.gif');
...
function update_runner() {
  ...
  update_timer = setTimeout(function () {
    if (page_active) {
      ...
      update_runner();
      change_upd_img('', 'https://www.bestchange.net/images/update.png');
    }
  }, ud * 15000);
}
...
function check_telegram_link() {
  ...
  telegram_status.innerHTML = '<img src="https://www.bestchange.net/images/ajax.gif" width="16" height="16" alt= />';
...
function update_stats(forcibly) {
  ...
  if (forcibly || chartcanvas.className == 'waitchart' || new Date() - chart_time > 2 * 60 * 1000) {
    ...
    var chart_url = 'https://www.bestchange.net/chart.php?type=' + stats_type + '&ci=' + stats_interval;
    if (ud_page == 'rates') chart_url += '&from=' + lc_curr + '&to=' + rc_curr;
    chart_url += '&rnd=' + Math.random() + '&session=' + session_params;

    chartcanvas.innerHTML = '<div class="waitborder"></div> <img src="https://www.bestchange.net/images/ajax-big.gif" class="waitimg" alt="Ждите..." /> <img src="' + chart_url + '" class="chartimg" onload="chart_update_complete(true)" onerror="chart_update_complete(false)" />';
  }
  return false;
}
...
function show_addform(locate, name, u, t) {
  ...
  if (show_add_name == 'social') {
    ...
    for (var i = 0; i < social_data.length; i += 3) content += '<a target="_blank" href="http://' + social_data[i] + '/' + social_data[i + 1].replace('{u}', u).replace('{t}', t) + '" title="Добавить в ' + social_data[i + 2] + '"><img src="https://www.bestchange.net/images/social/' + social_data[i] + '.png" width="16" height="16" alt="' + social_data[i + 2] + '" /></a>';
...
function start_bonus_timer() {
  ...
  if (left <= 0) {
    ...
    if (bonus_timer_count > 5 && !page_active) {
      var sound = new Audio();
      sound.src = 'https://www.bestchange.net/images/notify.mp3';
      sound.autoplay = true;
      bonus_timer_count = 0;
    }
    return;
...
function get_bonus() {
  ...
  if (req) {
    ...
    bonus_result.innerHTML = '<img src="https://www.bestchange.net/images/ajax-big.gif" width="48" height="48" style="padding: ' + (height / 2 - 28) + 'px 0 0 ' + (width / 2 - 24) + 'px" alt="Ждите..." />';
    clearTimeout(bonus_captcha_timer);
    ...
    req.onreadystatechange = function() {
      ...
      if (req.status == 200) {
        ...
        if (results.length == 6) {
          if (results[0] == 'OK') {
            set_bonus_status(results[5] + '<p>Если вам понравился наш мониторинг обменников, вы можете:</p><ul><li>Написать свой отзыв на <a target="_blank" rel="noreferrer" href="https://www.mywot.com/ru/scorecard/bestchange.com">MyWOT</a> или во <a target="_blank" href="https://vk.com/topic-53507345_39091228">ВКонтакте</a> (мы очень любим получать отзывы);</li><li><a href="/partner/">Зарегистрироваться</a> в нашей партнерской программе и зарабатывать деньги, <span class="videolink" onclick="show_video(\'aff-ru\')"><span class="videoicon"></span><span class="link dashlink">рекламируя</span></span> BestChange;</li><li>Посоветовать BestChange вашим друзьям и знакомым;</li><li>Подписаться на нашу группу ВКонтакте:</li></ul><iframe src="/js/vk.html" align="middle" width="266" height="250" allowTransparency="allowTransparency" scrolling="no" border="0" frameborder="0" seamless="seamless"></iframe>', results[1], 'ok.png');
...
function load_overlay(params) {
  ...
  if (req) {
    ...
    lightboxContent.innerHTML = '<img src="https://www.bestchange.net/images/ajax-big.gif" width="48" height="48" style="padding: ' + (height / 2 - 28) + 'px 0 0 ' + (width / 2 - 24) + 'px" alt="Ждите..." />';
...
function show_video(id) {
  ...
  show_overlay();
  var lightboxContent = document.getElementById('lightboxContent');
  lightboxContent.innerHTML = '<img src="https://www.bestchange.net/images/ajax-big.gif" id="video_pre_load" width="48" height="48" style="padding: ' + (lightboxContent.offsetHeight / 2 - 28) + 'px 0 0 ' + (lightboxContent.offsetWidth / 2 - 24) + 'px" alt="Ждите..." /><div id="video_frame"><iframe onload="setTimeout(function () {video_load_complete()}, 200)" width="600" height="338" src="https://www.youtube.com/embed/' + video + '?rel=0&autoplay=1&showinfo=0" frameborder="0" allowfullscreen></iframe></div>';
}
...
function es_click(from, to) {
  openDocument('https://www.bestchange.net/' + cu_list[id2pos(from)] + '-to-' + cu_list[id2pos(to)] + '.html', false);
}
...
function select_country(new_country, direct) {
  ...
  for (var i = 0; i < citylist.length; i++) {
    if (citylist[i][0] == new_country) {
      city_lines += '<a href="https://www.bestchange.net/' + direct + '-in-' + citylist[i][3] + '.html" id="city' + citylist[i][1] + '"' + (citylist[i][1] == city ? ' class="cityselected"' : '') + '>' + citylist[i][2] + (citylist[i][4] > 0 ? '<span>' + citylist[i][4] + '</span>' : '') + '</a>';
    }
  }
...
document.write('<style type="text/css">input.styled { display: none; } input[type="checkbox"] { position: absolute; left: -9999px; } select.styled { opacity: 0 !important; filter: alpha(opacity=0) !important; -khtml-opacity: 0 !important; -moz-opacity: 0 !important; } #curr_list select.styled {width: 190px; font-size: 13px} @-moz-document url-prefix() { #curr_list optgroup { padding-left: 2px; } #curr_list option { background: url("https://www.bestchange.net/images/systems.png") no-repeat; padding: 2px 0 2px 26px; } }</style>');
